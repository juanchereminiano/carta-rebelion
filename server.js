...
app.get('/api/thinkion/diagnostico', requireAdmin, (req, res) => {
  try {
    ...
    const dataDir = process.env.THINKION_DATA_DIR
      || path.join(__dirname, 'data', 'thinkion-cache');
    const empDir  = path.join(dataDir, empresaId);

    let archivos = [];
    if (require('fs').existsSync(empDir)) {
      archivos = require('fs').readdirSync(empDir)
        .filter(f => f.endsWith('.json'))
        .map(f => {
          const full  = path.join(empDir, f);
          const stat  = require('fs').statSync(full);
          const filas = (() => { try { return JSON.parse(require('fs').readFileSync(full,'utf8')).length; } catch { return '?'; } })();
          return { archivo: f, filas, guardado: stat.mtime.toISOString().slice(0,10) };
        })
        .sort((a, b) => b.archivo.localeCompare(a.archivo));
    }

    const appCache = cache.get(`carta_${empresaId}`);
    res.json({
      ok: true,
      empresa:        empresaId,
      meses_en_disco: archivos.length,
      archivos,
      app_cache:      appCache ? `${appCache.records.length} records en memoria` : 'vacío',
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
